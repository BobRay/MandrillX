...
class MandrillX extends Mandrill{
    ...
    function __construct(&$modx, $apiKey = null, &$config = array()) {
        /** @var $modx modX */
        $this->modx =& $modx;
        $this->config =& $config;
        if ($apiKey == null) {
            $apiKey = $modx->getOption('mandrill_api_key', $config, null);
        }
    ...
    public function sendMessage() {
        ...
        $this->message['html'] = $this->_prepareTpl($this->message['html']);
        if( !empty($this->message['text'])) {
            $this->message['text'] = $this->_prepareTpl($this->message['text']);
        }
        $retVal = 'Error';
        try {
